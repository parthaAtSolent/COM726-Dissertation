...
class DeepSeekStreamingHandler:
...
def build(streaming: bool = False, on_token: Optional[Callable[[str], None]] = None) -> ChatOllama:
    """
    Build DeepSeek-R1 model with optional streaming support.
    ...
    if streaming:
        ...
        if on_token:
            handler = DeepSeekStreamingHandler(on_token)
...
class DeepSeekR1:
    """
    Advanced streaming wrapper for DeepSeek-R1.
    Provides both sync and async streaming with automatic reasoning tracking.
    """

    def __init__(self):
        self.model = build(streaming=False)  # Base model

    def stream(self, prompt: str, show_thinking: bool = True) -> Iterator[str]:
        """
        Stream response with intelligent chunking.

        Args:
            prompt: User input
            show_thinking: Show reasoning steps (DeepSeek-R1 specific)
        """
        full_response = ""
        buffer = ""
        ...
        for chunk in self.model.stream(prompt):
            ...
            # Detect reasoning tags (DeepSeek-R1 specific)
            if show_thinking and 'thinking' in content.lower():
                yield f"\n🧠 Reasoning: {content}\n"
    ...
    async def astream(self, prompt: str, show_thinking: bool = True) -> AsyncIterator[str]:
        """
...
def get_instance() -> DeepSeekR1:
    """Get or create singleton DeepSeekR1 instance."""
    global _deepseek_instance
    if _deepseek_instance is None:
        _deepseek_instance = DeepSeekR1()
