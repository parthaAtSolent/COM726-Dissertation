...
def build(streaming: bool = False, on_token: Optional[Callable[[str], None]] = None) -> ChatOllama:
    """
    Build Phi-3 3.8B model with optional streaming support.
    ...
    if streaming:
        ...
        if on_token:
            handler = Phi3StreamingHandler(on_token)
...
class Phi3_38B:
    """
    Advanced streaming wrapper for Phi-3 3.8B.
...
def get_instance() -> Phi3_38B:
    """Get or create singleton Phi3_38B instance."""
    global _phi3_instance
    if _phi3_instance is None:
        _phi3_instance = Phi3_38B()
