...
def build(streaming: bool = False, on_token: Optional[Callable[[str], None]] = None) -> ChatOllama:
    """
    Build Gemma 3 270M model with optional streaming support.
    ...
    if streaming:
        ...
        if on_token:
            handler = Gemma3StreamingHandler(on_token)
...
class Gemma3270M:
    """
    Advanced streaming wrapper for Gemma 3 270M.
...
_gemma3_instance = None


def get_instance() -> Gemma3270M:
    """Get or create singleton Gemma3270M instance."""
    global _gemma3_instance
    if _gemma3_instance is None:
        _gemma3_instance = Gemma3270M()
    return _gemma3_instance
