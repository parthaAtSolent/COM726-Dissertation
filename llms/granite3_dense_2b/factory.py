...
class Granite3StreamingHandler:
...
def build(streaming: bool = False, on_token: Optional[Callable[[str], None]] = None) -> ChatOllama:
    """
    Build Granite 3 Dense 2B model with optional streaming support.
    ...
    if streaming:
        ...
        if on_token:
            handler = Granite3StreamingHandler(on_token)
            config["callbacks"] = [handler]

    return ChatOllama(**config)


class Granite3Dense2B:
    """
    Advanced streaming wrapper for Granite 3 Dense 2B.
...
_granite3_instance = None


def get_instance() -> Granite3Dense2B:
    """Get or create singleton Granite3Dense2B instance."""
    global _granite3_instance
    if _granite3_instance is None:
        _granite3_instance = Granite3Dense2B()
    return _granite3_instance
