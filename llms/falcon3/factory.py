...
def build(streaming: bool = False, on_token: Optional[Callable[[str], None]] = None) -> ChatOllama:
    """
    Build Falcon 3 model with optional streaming support.
    ...
    if streaming:
        ...
        if on_token:
            handler = Falcon3StreamingHandler(on_token)
...
class Falcon3:
    """
    Advanced streaming wrapper for Falcon 3.
...
_falcon3_instance = None


def get_instance() -> Falcon3:
    """Get or create singleton Falcon3 instance."""
    global _falcon3_instance
    if _falcon3_instance is None:
        _falcon3_instance = Falcon3()
    return _falcon3_instance
