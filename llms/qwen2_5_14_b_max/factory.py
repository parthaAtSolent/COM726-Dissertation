...
def build(streaming: bool = False, on_token: Optional[Callable[[str], None]] = None) -> ChatOllama:
    """
    Build Qwen 2.5 14B Max model with optional streaming support.
    ...
    if streaming:
        ...
        if on_token:
            handler = Qwen25StreamingHandler(on_token)
            config["callbacks"] = [handler]

    return ChatOllama(**config)


class Qwen25_14B_Max:
    """
    Advanced streaming wrapper for Qwen 2.5 14B Max.
...
_qwen25_instance = None


def get_instance() -> Qwen25_14B_Max:
    """Get or create singleton Qwen25_14B_Max instance."""
    global _qwen25_instance
    if _qwen25_instance is None:
        _qwen25_instance = Qwen25_14B_Max()
